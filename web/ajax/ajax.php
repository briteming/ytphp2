...
ini_set('error_log', dirname(__FILE__) . '/error_log.txt');  
    ...
    switch($type){
    	case 'video':
    		    ...
    		    echo'<div class="alert alert-danger h4 p-3 m-2" role="alert">Sorry, Content not found<strong>'.urldecode($q).'</strong>Related videos</div>';
    		    exit;
    		}
            ...
            if (array_key_exists("nextPageToken",$videodata) && array_key_exists("prevPageToken",$videodata) ) {
                ...
                echo'<a class="btn btn-outline-primary  w-25 pull-left" href="./search.php?q='.$_GET["q"].'&order='.$_GET["order"].'&type='.$_GET['type'].'&pageToken='.$videodata["prevPageToken"].'" data-toggle="">Back</a>
                ...
                echo '<a class="btn btn-outline-primary btn-block" href="./search.php?q='.$_GET["q"].'&order='.$_GET["order"].'&type='.$_GET['type'].'&pageToken='.$videodata["prevPageToken"].'" data-toggle="">Back</a>' ;
            }
            echo'</div>';
    		break;
        case 'recommend':
      ...
      <div class="media-body col-8 chaneelit">
    	...
    	case 'channels':
    		...
    		if($video['pageInfo']['totalResults']<=1){
    		    echo'<p>Failed to get content! This channel user has not uploaded any content, or the channel content is protected by copyright and cannot be viewed at this time!</p>';
    		    exit;
    		}
    ...
    case 'menu':
        ...
        <li class="list-group-item font-weight-bold"><i class="fa fa-home fa-fw pr-4"></i><a href="./index.php" class="text-dark">Home</a></li>
        ...
        <li class="list-group-item"><i class="fa fa-cloud-download fa-fw pr-4"></i><a href="./content.php?cont=video" class="text-dark">YT Download</a></li>
        </ul>
        <ul class="list-group pt-3">
        <li class="list-group-item font-weight-bold"></i>YT Category</li>
        ';
    ...
    case 'DMCA':
        echo '<div class="font-weight-bold h6 pb-1">DMCA Disclaimer</div>';
        echo '<h6><b>To whom it may concern：</b><h6>';
        echo '<p class="h6" style="line-height: 1.7">This site video content from the Internet.<br>
If any content violates your copyright.<br>
...
echo '<h6 class="pt-3"><b>用户须知：</b><h6>';
     ...
     case 'api':
         ...
         break;
    case 'videos':
        echo '<div class="font-weight-bold h6 pb-1">YT Downloader</div>';
        echo '<form  onsubmit="return false" id="ipt">
  <div class="form-group text-center" >
  <input name="type" value="videodownload" style="display: none;">
      <input type="text" name="link"  placeholder="Fill in Youtube link here" id="soinpt"  autocomplete="off" /><button type="submit" id="subu" style="width: 24%;vertical-align:middle;border: none;height: 50px;background-color: #e62117;color: #fff;font-size: 18px;display: inline-block;" ><i class="fa fa-download fa-lg pr-1"></i>Download</button>
  </div>
    </form>';
    if(isset($_GET['type']) && isset($_GET['v'])){
       ...
       $viddata=get_video_info($_GET['v'],APIKEY);
        ...
        echo '<div id="videoslist" class="text-center"><p>Tip: If you cant download, please select right-click Save As..!<p></div>'; 
    }
    echo '<script>
