...
//分享功能
function shareit($id,$title='YT Link Sharing'){
    $pic=ROOT_PART.'/thumbnail.php?vid='.$id;
    $url=ROOT_PART.'watch-'.$id.'.html';
    $title=str_replace('&','||',$title);
    $title=str_replace('"',' ',$title);
     $title=str_replace("'",' ',$title);
    $des='I am watching video《'.$title.'》!';
    return "<div id='share'>
  <a class='icoqz' href='https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=".$url."&desc=".$des."&title=".$titlel."
&pics=".$pic."' target='blank' title='Share to QQ Moment'><i class='iconfont icon-qqkongjian icofontsize'></i></a>

  <a class='icotb' href='http://tieba.baidu.com/f/commit/share/openShareApi?title=".$title."&url=".$url."&to=tieba&type=text&relateUid=&pic=".$pic."&key=&sign=on&desc=&comment=".$title."' target='blank' title=Share'><i class='iconfont icon-40 icofontsize'></i></a>

  <a class='icowb' href='http://service.weibo.com/share/share.php?url=".$url."&title=".$des."&pic=".$pic."&sudaref=".$title."' target='blank' title='Share to wechat'><i class='iconfont icon-weibo icofontsize'></i></a>
 ...
 <div class='form-group'><div class='d-inline-block h6 pt-3 col-12'>
    ...
    <button type='submit' class='btn btn-primary align-middle col-12 mt-2' onclick='copytext1()'>Send</button></div>";
...
function html5_player($id){
    ...
    //提取字幕
     ...
     if(array_key_exists('track',$array1) && array_key_exists('0',$array1['track'])){
    	...
    	foreach ($array1['track'] as $val) {if ($val['@attributes']['lang_code'] == 'en' || $val['@attributes']['lang_code'] == 'zh' || $val['@attributes']['lang_code'] =='zh-CN' || $val['@attributes']['lang_code'] =='vi-VN' || $val['@attributes']['lang_code'] =='zh-HK') {
